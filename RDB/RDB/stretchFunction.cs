...
namespace RDB
{
    public partial class stretchFunction : Form
    {
        ...
        double MaxPercent=1;
        double MinPercent=0;
        double NumberOfStandardDeviations=2;
        XmlNode m_xmlNode;
        ...
        public stretchFunction(XmlNode in_xmlNode)
        {
            ...
            foreach (XmlNode node in childlist)
            {
                ...
                if (element.Name == "MaxPercent") MaxPercent = int.Parse(element.InnerText);
                if (element.Name == "MinPercent") MinPercent = int.Parse(element.InnerText);
                if (element.Name == "NumberOfStandardDeviations") NumberOfStandardDeviations = int.Parse(element.InnerText);
            }
                cmb_StretchType.Items.Add("None");
                cmb_StretchType.Items.Add("StandardDeviation");
                cmb_StretchType.Items.Add("HistogramEqualization");
